...
func ValidateToken(token string, secret string) (bool, error) {
	...
	if len(splitToken) != 3 {
		return false, errors.New("token incorrect")
	}
	...
	limitS := data["exp"].(string)
	limitInt, _ := strconv.Atoi(limitS)

	if isExpired(limitInt) {
		return false, errors.New("token epired")
	}
...
func isExpired(expValue int) bool {
	return expValue < int(time.Now().Unix())
}
